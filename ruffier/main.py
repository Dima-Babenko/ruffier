...
name = ""
p1, p2, p3 = 0, 0, 0
liters = "йцукенгшщзхїфівапролджєячсмитьбю qwertyuiopasdfghjklzxcvbnm"
...
class InstrScr(Screen):
    def __init__(self, **kwargs):
        ...
        self.add_widget(Image(source='1.jpg', allow_stretch=True, keep_ratio=False))
    ...
    def next(self):
        ...
        if len(name) == 0:
            self.in_name.background_color = 'red'
            return
        for sym in name:
            if sym.lower() not in liters:
                self.in_name.background_color = 'red'
                return
        age = check_int(self.in_age.text)
        if age < 7:
            age = 7
            self.in_age.background_color = 'red'
            self.in_age.text = '7'
            return
...
class PulseScr(Screen):
    ...
    def next(self):
        ...
        if not self.next_screen:
            ...
            if p1 < 1:
                p1 = ""
                self.in_result.background_color = "red"
                self.in_result.text = "0"
            else:
                self.in_result.background_color = "green"
...
class CheckSits(Screen):
    def __init__(self, **kwargs):
        ...
        self.run = Runner(30, 1.5, size_hint=(0.2, 1))
...
class PulseScr2(Screen):
    ...
    def next(self):
        ...
        if not self.next_screen:
            ...
            if p2 <= 0:
                p2 = "0"
                self.in_result1.background_color = "red"
                return
            self.in_result1.background_color = "green"
            p3 = check_int(self.in_result2.text)
            if p3 <= 0:
                p3 = "0"
                self.in_result2.background_color = "red"
                return
            self.in_result2.background_color = "green"
